...
function ethInit(account, contractAddress, gasPrice) {
...
function payout(contract, addresses, amounts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    contract.methods.distributeTokens(addresses, amounts).estimateGas().then(gasEstimate => {
      ...
      console.log(`sending tx for distributeTokens() with gas limit ${gasLimit}, gasPrice ${contract.options.gasPrice} and params:
            ${addresses},
            ${amounts}`);
      if (process.env.ETH_FOR_REAL) {
...
const ethPromise = ethInit(
  process.env.ETH_ACCOUNT || "0xFe4E89f620a8663d03136bee040904fe3A623f5D",
  process.env.ETH_P4P_ADDR || "0x78Cb0dB58721596Bc79Dc9D8d8296212D153D804",
  process.env.ETH_GASPRICE_GWEI * 1E9 + 7 || 4 * 1E9 + 7 // default: 4 gwei. +7 for frontrunning
);
...
Promise.all([ethPromise, dbPromise]).then( (results) => {
  console.log("lets go!");
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    promiseChain = promiseChain.then(payout.bind(null, contract, addresses, amounts));
  }
});