...
const debug = gs.inDebugMode();

const log = debug ? (msg) => { console.log(`EthereumUtils: ${msg}`); } : () => {};
const err = debug ? (msg) => { console.error(`ERROR in EthereumUtils: ${msg}`); } : () => {};
...
class EthereumUtils {
  ...
  unlockWallet(password) {
    try {
      ...
      if (this.wallet.length === 0) {
        err('There is no wallet stored we could unlock');
        return null;
      }
  ...
  getAddress() {
    if (this.wallet) {
      /* if a locked wallet is loaded from localStorage, the address is formatted differently
      then when loaded unlocked via web3. */
  ...
  needsPersist() {
    return this.wallet && !this.wallet.persisted;
  }
  ...
  loadWallet(forceNew) {
    if (!forceNew) {
      ...
      try {
        ...
        This will return an empty wallet if nothing is stored and throw an exception (password missing)
        if a wallet is stored.
        ...
        /* Thus if neither an Exception was thrown nor forceNew set,
        we will just proceed with a new wallet (after catch block). */
      } catch (e) {
